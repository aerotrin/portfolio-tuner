...
from src.services.streamlit_data import delete_transaction
...
def render_closed_lots_table(
    ...
    if not df_filtered.empty:
        ...
        with metrics[2]:
            if account_status == "Non-Registered":
                st.metric("Capital Gain/Loss", f"${df_filtered['gain'].sum():,.2f}")
            else:
                st.metric("Gain/Loss", f"${df_filtered['gain'].sum():,.2f}")
        with metrics[3]:
...
def confirm_delete_transaction_dialog(account_number: str, transaction_id: str) -> None:
    st.warning(
    ...
    with col2:
        if st.button("Delete", type="primary", width="stretch"):
            msg = delete_transaction(account_number, transaction_id)
            st.success(msg)
            st.cache_data.clear()
            st.session_state.pop("pending_delete_transaction_id", None)
            st.rerun()
...
def render_transactions_table(
    ...
    if pending_id:
        confirm_delete_transaction_dialog(
            st.session_state["account_number"], pending_id
        )
