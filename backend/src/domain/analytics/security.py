...
def compute_portfolio_timeseries_indicators(
    ...
    on top of that (daily_return, ema10/50/100, rsi). Returns a DataFrame indexed by date.
    """
    if not securities:
        return [pd.DataFrame()]
    ...
    for portfolio in close_matrix.columns:
        ...
        df["ema10"] = ema10[portfolio]
        df["ema50"] = ema50[portfolio]
...
def compute_performance_metrics(
    ...
    # merton = _calc_merton(sharpe, MERTON_CRA, volatility)
    max_drawdown, max_drawdown_date = _calc_max_drawdown(daily)
    trend = _calc_trend(short_term_returns, volatility)
    flags = _calc_flags(close, ema10, NEAR_HIGH_LOW_PCT, TRADING_DAYS)
...
def _calc_flags(
    ...
    Add boolean flags to a 1-row dict:
      - above_ema: latest close > EMA_mid
    ...
    if close.empty or ema.empty:
        return {"above_ema": pd.NA, "near_52wk_hi": pd.NA, "near_52wk_lo": pd.NA}

    close_now = close.iloc[-1]
    ema_mid_now = ema.iloc[-1]
    if pd.notna(close_now) and pd.notna(ema_mid_now):
        out["above_ema"] = bool(close_now > ema_mid_now)
    else:
        out["above_ema"] = pd.NA
    ...
    if pd.notna(hi52) and hi52 > 0 and pd.notna(close_now):
        near_hi = (hi52 - close_now) / hi52 <= thr
        out["near_52wk_hi"] = bool(near_hi)
    else:
        out["near_52wk_hi"] = pd.NA

    if pd.notna(lo52) and lo52 > 0 and pd.notna(close_now):
        near_lo = (close_now - lo52) / lo52 <= thr
        out["near_52wk_lo"] = bool(near_lo)
    else:
        out["near_52wk_lo"] = pd.NA
    return out
...
# def _calc_merton(sr: float, cra: float, vol: float) -> float:
#     """
#     Merton ratio from a Sharpe ratio, Conditional Risk Aversion, and volatility.
#     """
#     if vol <= 0.0 or cra <= 0.0:
#         return np.nan
#     return sr / (cra * vol)
# FTRK-335 - not used and deprecated
...
def _calc_trend(short_term_returns: dict[str, float], volatility: float) -> float:
    """
    Compute a raw short-term trend strength (-1 to 1).
    """
    if not short_term_returns or volatility == 0.0:
        return np.nan
    return float(short_term_returns["return5D"]) / volatility
