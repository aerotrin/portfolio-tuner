...
Symbol sets for the market research pages, benchmarks, and the market snapshot header are configured in `symbols.yml` at the project root. The symbols loader (`shared/symbols_loader.py`) parses this YAML file into a validated `SymbolsConfig` Pydantic model. The bootstrap phase reads this file once and stores the derived symbol sets in session state. Changing symbols only requires editing the YAML file — no code changes.

Each group carries three attributes that form the filter taxonomy: a required `label`, plus an optional `type` (Equity, Fixed Income, Commodity, …) and `region` (US, Canada, World, N/A). Both optional fields default to `Other`/`N/A`, so configs predating them keep loading. A group is identified by `SymbolGroup.key`, the slug of region + type + label; the loader rejects duplicates of that triple, and widget keys derive from it. Filtering, however, selects on `label` alone: a label names a concept that deliberately repeats across regions ("Core" exists under US, Canada and World), so choosing "Sectors" with no region selected means every region's Sectors group, and the region and type facets are what narrow it. `widgets/filters.py` turns these into the cascading Region → Type → Group filter on the market Performance tab; its cascade is a pure function (`_cascade`) that drops selections orphaned by a change at a higher level. The Intraday tab consumes the same taxonomy as treemap structure instead — region → group → symbol nesting with click-to-zoom — scoped by type pills and a group multiselect.
